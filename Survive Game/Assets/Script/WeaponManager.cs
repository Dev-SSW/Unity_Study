...
{
    //무기 교체 시 중복 교체를 방지를 위해 static 사용
    ...
    void Update()
    {
        if(!isChangeWeapon)
        {
            if (Input.GetKeyDown(KeyCode.Alpha1))
                StartCoroutine(ChangeWeaponCoroutine("HAND", "맨손"));//무기 교체 실행 (서브 머신건)
                ...
                StartCoroutine(ChangeWeaponCoroutine("PICKAXE", "Pickaxe")); //무기 교체 실행 (Pickaxe)
        }
    }
